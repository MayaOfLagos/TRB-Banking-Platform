            ...
            <li class="dropdown d-flex profile-dropdown">
                <button type="button" data-bs-toggle="dropdown" data-display="static" aria-haspopup="true" aria-expanded="false">
                    <span class="navbar-user">
                        <span class="navbar-user__thumb"><img src="{{ getImage(getFilePath('adminProfile') . '/' . auth()->guard('admin')->user()->image, null, true) }}" alt="image"></span>
...
@php
    foreach ($settings as $setting){
        if(is_array($setting->route_name)){
            foreach ($setting->route_name as $route) {
                if(can($route)){
                    $setting->route_name = $route;
                    break;
                }
            }
        }
