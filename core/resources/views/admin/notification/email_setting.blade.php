...
@section('panel')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <form method="POST" action="{{ route('admin.setting.notification.email.update') }}">
                    @csrf
                    @php
                        $mailConfig = gs('mail_config');
                        if (is_string($mailConfig)) {
                            $mailConfig = json_decode($mailConfig);
                        }
                    @endphp
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select name="email_method" class="select2 form-control" data-minimum-results-for-search="-1">
                                <option value="php" @if ($mailConfig->name == 'php') selected @endif>@lang('PHP Mail')</option>
                                <option value="smtp" @if ($mailConfig->name == 'smtp') selected @endif>@lang('SMTP')</option>
                                <option value="laravel" @if ($mailConfig->name == 'laravel') selected @endif>@lang('Laravel Mail')</option>
                                <option value="sendgrid" @if ($mailConfig->name == 'sendgrid') selected @endif>@lang('SendGrid API')</option>
                                <option value="mailjet" @if ($mailConfig->name == 'mailjet') selected @endif>@lang('Mailjet API')</option>
                            </select>
                        </div>
...
@push('script')
    <script>
        (function($) {
            ...
            var method = '{{ $mailConfig->name ?? 'php' }}';
            emailMethod(method);
