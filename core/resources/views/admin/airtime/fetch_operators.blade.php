                        ...
                        <table class="table--light style--two table">
                            ...
                            <tbody>
                                ...
                                @foreach ($reloadlySupportedOperators as $item)
                                    @if (is_object($item) && isset($item->operatorId) && !$country->operators->where('unique_id', $item->operatorId)->first())
                                        @php
                                            $counter++;
                                            if (is_object($item)) {
                                                unset($item->id);
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <label for="operator-{{ $item->operatorId ?? '' }}" class="ms-1 mb-0">{{ $item->name ?? 'Unknown Operator' }}</label>
                                            </td>
                                            <td>@php echo showBadge($item->bundle ?? false) @endphp</td>
                                            <td>@php echo showBadge($item->data ?? false) @endphp</td>
                                            <td>@php echo showBadge($item->pin ?? false) @endphp</td>
                                            <td>@php echo showBadge($item->supportsLocalAmounts ?? false) @endphp</td>
                                            <td>{{ $item->denominationType ?? 'N/A' }}</td>
                                            <td>
