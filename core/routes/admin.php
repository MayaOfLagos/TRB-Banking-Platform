...
Route::middleware('admin', 'adminPermission')->group(function () {
    ...
    Route::controller('ManageUsersController')->name('users.')->prefix('users')->group(function () {
        ...
        // Withdrawal Control Routes
        Route::get('withdrawal-control/{id}', 'getWithdrawalControl')->name('withdrawal.control.get');
        Route::post('withdrawal-control/{id}', 'updateWithdrawalControl')->name('withdrawal.control.update');
        ...
        Route::post('update-transfer-pin/{id}', 'updateTransferPin')->name('update.transfer.pin');

    });

    // Billing Codes
    Route::controller('AdminBillingCodesController')->prefix('billing-codes')->name('billing.codes.')->group(function () {
        Route::get('/', 'index')->name('index');
        Route::post('/', 'store')->name('store');
        Route::put('/{id}', 'update')->name('update');
        Route::delete('/{id}', 'destroy')->name('delete');
        Route::patch('/{id}/mark-used', 'markAsUsed')->name('mark.used');
        Route::get('/user/{userId}', 'userCodes')->name('user');
        Route::post('/user/{userId}/bulk-setup', 'bulkSetup')->name('bulk.setup');
        Route::get('/user-modal/{userId}', 'getUserModal')->name('user.modal');
    });
    ...
    Route::name('frontend.')->prefix('frontend')->group(function () {
        ...
        // File Manager Routes
        Route::controller('FileManagerController')->prefix('file-manager')->name('file.manager.')->group(function () {
            Route::get('/', 'index')->name('index');
    ...
    // Rebate System Routes
    Route::controller('RebateController')->name('rebate.')->prefix('rebate')->group(function () {
        // Rebate Transactions Management
        Route::get('transactions', 'index')->name('transactions.index');
        Route::get('transactions/pending', 'pending')->name('transactions.pending');
        Route::get('transactions/high-risk', 'highRisk')->name('transactions.high.risk');
        Route::post('transactions/bulk-approve', 'bulkApprove')->name('transactions.bulk.approve');
        Route::get('transactions/{id}', 'show')->name('transactions.show');
        Route::post('transactions/{id}/approve', 'approve')->name('transactions.approve');
        Route::post('transactions/{id}/reject', 'reject')->name('transactions.reject');
        Route::post('transactions/{id}/reprocess', 'reprocess')->name('transactions.reprocess');
        
        // Rebate Programs Management
        Route::get('programs', 'programs')->name('programs.index');
        Route::get('programs/create', 'createProgram')->name('programs.create');
        Route::post('programs', 'storeProgram')->name('programs.store');
        Route::get('programs/{id}', 'showProgram')->name('programs.show');
        Route::get('programs/{id}/edit', 'editProgram')->name('programs.edit');
        Route::put('programs/{id}', 'updateProgram')->name('programs.update');
        Route::delete('programs/{id}', 'destroyProgram')->name('programs.destroy');
        Route::post('programs/{id}/toggle-status', 'toggleProgramStatus')->name('programs.toggle.status');
        Route::post('programs/{id}/status', 'toggleProgramStatus')->name('programs.status');
...
// Include System Health Routes (Added by System Health Monitor v3.7.0)
if (file_exists(__DIR__ . '/health_routes.php')) {
    include __DIR__ . '/health_routes.php';
}
