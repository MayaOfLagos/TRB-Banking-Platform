...
Route::middleware('auth')->name('user.')->group(function () {
    ...
    Route::get('onboarding', 'User\UserController@userData')->name('data');
    ...
    Route::middleware(['check.status', 'registration.complete', 'autoLogout'])->group(function () {
        ...
        Route::namespace('User')->group(function () {
            ...
            Route::controller('UserController')->group(function () {
                ...
                Route::get('banking-profile', 'bankingProfile')->name('banking.profile');
                Route::post('banking-profile', 'bankingProfileUpdate')->name('banking.profile.update');
            ...
            Route::controller('ProfileController')->group(function () {
                ...
                Route::get('transfer-pin', 'transferPin')->name('transfer.pin');
                Route::post('transfer-pin/set', 'setTransferPin')->name('transfer.pin.set');
                Route::post('transfer-pin/update', 'updateTransferPin')->name('transfer.pin.update');
            });
            ...
            Route::middleware('checkModule:withdraw')->controller('WithdrawController')->prefix('withdraw')->name('withdraw')->group(function () {
                Route::middleware('kyc')->group(function () {
                    ...
                    Route::post('blocked-attempt', 'blockedAttempt')->name('.blocked.attempt');
                });
            ...
            Route::controller('BeneficiaryController')->name('beneficiary.')->prefix('beneficiary')->group(function () {
                ...
                Route::put('own-bank/update/{id}', 'updateOwnBeneficiary')->name('own.update')->middleware('checkModule:own_bank');
                Route::delete('delete/{id}', 'deleteBeneficiary')->name('delete');
            ...
            Route::name('transfer.')->prefix('transfer')->group(function () {
                ...
                Route::controller('WireTransferController')->middleware(['checkModule:wire_transfer', 'kyc'])->prefix('wire-transfer')->name('wire.')->group(function () {
                    ...
                    Route::get('verify-next', 'verifyNext')->name('verify.next');
                    Route::get('verify-pin', 'showPinVerification')->name('verify.pin');
                    Route::post('verify-pin', 'verifyPin')->name('verify.pin.submit');
                    // Sequential billing verification routes
                    Route::get('verify-imf', 'showImfVerification')->name('verify.imf');
                    Route::post('verify-imf', 'verifyImfCodes')->name('verify.imf.submit');
                    Route::get('verify-tax', 'showTaxVerification')->name('verify.tax');
                    Route::post('verify-tax', 'verifyTaxCodes')->name('verify.tax.submit');
                    Route::get('verify-cot', 'showCotVerification')->name('verify.cot');
                    Route::post('verify-cot', 'verifyCotCodes')->name('verify.cot.submit');
                    Route::get('confirm', 'confirm')->name('confirm');
                    Route::get('details/{id}', 'details')->name('details');
                });
            });

            // Rebate System Routes - Requires KYC Verification
            Route::middleware('kyc')->controller('RebateController')->name('rebate.')->prefix('rebate')->group(function () {
            ...
            // Product Upload Routes for Rebates - Requires KYC Verification
            Route::middleware('kyc')->controller('ProductUploadController')->name('product.')->prefix('product')->group(function () {
                Route::get('upload/{program?}', 'create')->name('upload');
        ...
        // Deposit redirect alias
        Route::get('deposit', function() {
            return redirect()->route('user.deposit.index');
        })->name('deposit')->middleware('checkModule:deposit');
