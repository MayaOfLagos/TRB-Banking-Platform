            ...
            Operator::active()->chunk(20, function ($operators) use ($reloadly) {
                foreach ($operators as $operator) {
                    ...
                    if($item && is_object($item)) {
                        $operator->name                                  = $item->name ?? $operator->name;
                        $operator->bundle                                = isset($item->bundle) ? ($item->bundle ? 1 : 0) : 0;
                        $operator->data                                  = isset($item->data) ? ($item->data ? 1 : 0) : 0;
                        $operator->pin                                   = isset($item->pin) ? ($item->pin ? 1 : 0) : 0;
                        $operator->denomination_type                     = $item->denominationType ?? $operator->denomination_type;
                        $operator->destination_currency_code             = $item->destinationCurrencyCode ?? $operator->destination_currency_code;
                        $operator->destination_currency_symbol           = $item->destinationCurrencySymbol ?? $operator->destination_currency_symbol;
                        $operator->most_popular_amount                   = $item->mostPopularAmount ?? $operator->most_popular_amount;
                        $operator->min_amount                            = $item->minAmount ?? $operator->min_amount;
