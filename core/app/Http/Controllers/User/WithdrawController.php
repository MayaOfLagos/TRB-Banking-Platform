...
class WithdrawController extends Controller
{
    ...
    public function exportPDF(Request $request)
    {
        try {
            ...
            // Build query with same filters as withdrawLog
            $query = $user->withdrawals();
            
            // Store applied filters for PDF display
            $filters = [];
            ...
            if ($request->input('method')) {
                $query = $query->whereHas('method', function($q) use ($request) {
                    $q->where('name', 'like', '%' . $request->input('method') . '%');
                });
            ...
            $withdraws = $query->with('method', 'branch:id,name')
                             ->orderBy('id', 'desc')
                             ->get();
            
            // Calculate summary data
            $total_amount = $withdraws->sum('amount');
            $total_charge = $withdraws->sum('charge');
            $total_received = $withdraws->sum('after_charge');
            ...
            $data = [
                'pageTitle' => 'Withdrawal Statement - ' . $user->username,
                'user' => $user,
                'withdraws' => $withdraws,
                'filters' => $filters,
            ...
            // Use the existing PDFManager to generate PDF
            return downloadPdf('Template::pdf.withdrawal_statement', $data);
            ...
            // Return error for regular request
            $notify[] = ['error', 'Failed to generate PDF statement. Please try again.'];
    ...
    public function blockedAttempt(Request $request)
    {
        $user = auth()->user();
        $withdrawalControl = $user->withdrawalControl;
        
        if (!$withdrawalControl) {
            return response()->json(['success' => false, 'message' => 'No withdrawal control found']);
        }

        $withdraw = Withdrawal::where('trx', session()->get('wtrx'))
        ...
        if (!$withdraw) {
            return response()->json(['success' => false, 'message' => 'Withdrawal not found']);
        }

        // Update withdrawal status to indicate it was blocked
        $withdraw->status = Status::PAYMENT_REJECT;
        $withdraw->admin_feedback = 'Withdrawal blocked due to account restriction: ' . ($withdrawalControl->reason ?? 'N/A');
        $withdraw->save();

        // Get status label
        $statusLabels = \App\Models\WithdrawalControl::getStatuses();
        $statusLabel = $statusLabels[$withdrawalControl->status] ?? $withdrawalControl->status;

        // Send email notification to user
        notify($user, 'WITHDRAWAL_BLOCKED', [
            'status' => $statusLabel,
            'reason' => $withdrawalControl->reason ?? 'Your withdrawal has been restricted. Please contact support for assistance.',
            'amount' => showAmount($withdraw->amount, currencyFormat: false),
            'method_name' => $withdraw->method->name,
            'trx' => $withdraw->trx,
            'time' => showDateTime($withdraw->created_at, 'M d, Y h:i A'),
        ]);

        return response()->json([
            'success' => true,
            'message' => 'Withdrawal attempt logged and notification sent'
        ]);
    }
}
