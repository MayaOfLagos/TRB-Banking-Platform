...
class ManageUsersController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $availableCurrencies = array_keys(getAvailableCurrencies());
        ...
        $request->validate([
            ...
            'preferred_currency' => 'nullable|in:' . implode(',', $availableCurrencies),
        ]);
        ...
        if ($request->has('preferred_currency')) {
            $user->preferred_currency = $request->preferred_currency ? strtoupper($request->preferred_currency) : null;
        }
    ...
    public function getWithdrawalControl($id)
    {
        $user = User::with('withdrawalControl')->findOrFail($id);
        
        $control = $user->withdrawalControl;
        
        return response()->json([
            'success' => true,
            'data' => [
                'status' => $control ? $control->status : \App\Models\WithdrawalControl::STATUS_ALLOWED,
                'reason' => $control ? $control->reason : '',
                'set_by' => $control && $control->admin ? $control->admin->name : null,
                'updated_at' => $control ? $control->updated_at->format('M d, Y h:i A') : null,
            ]
        ]);
    }

    public function updateWithdrawalControl(Request $request, $id)
    {
        $request->validate([
            'status' => 'required|in:allowed,pending_review,on_hold,suspended,restricted',
            'reason' => 'required_unless:status,allowed|nullable|string|max:1000'
        ], [
            'reason.required_unless' => 'Please provide a reason for this withdrawal control status'
        ]);

        $user = User::findOrFail($id);
        
        $withdrawalControl = \App\Models\WithdrawalControl::updateOrCreate(
            ['user_id' => $user->id],
            [
                'status' => $request->status,
                'reason' => $request->reason,
                'set_by' => auth()->guard('admin')->id(),
            ]
        );
        ...
        $notify[] = ['success', "Withdrawal control updated to: {$statusLabel}"];
    ...
    public function sendNotificationSingle(Request $request, $id)
    {
        ...
        if ($request->via === 'push') {
            if (!gs('pn')) {
                $notify[] = ['warning', 'Push notifications are disabled currently'];
                return back()->withNotify($notify);
            }
            ...
            if ($request->hasFile('image')) {
                $imageName = fileUploader($request->image, getFilePath('push'));
                $imageUrl = asset(getFilePath('push') . '/' . $imageName);
            }
            ...
            $payload = [
                ...
                'meta' => __('Sent by :admin', ['admin' => $adminName]),
            ...
            $queued = app(NotificationService::class)->sendPushNotification(
                $user->id,
                $request->subject,
                $request->message,
                $payload
            ...
            if (!$queued) {
                $notify[] = ['error', 'Failed to queue push notification'];
                return back()->withNotify($notify);
            }

            $notificationLog = new NotificationLog();
            $notificationLog->user_id = $user->id;
            $notificationLog->sender = $adminName;
            $notificationLog->sent_from = 'Admin Panel';
            $notificationLog->sent_to = $user->username;
            $notificationLog->subject = $request->subject;
            $notificationLog->message = $request->message;
            $notificationLog->notification_type = 'push';
            $notificationLog->image = $imageName;
            $notificationLog->save();

            $notify[] = ['success', 'Push notification queued successfully'];
            return back()->withNotify($notify);
        }
    ...
    public function updatePassword(Request $request, $id)
    {
        ...
        if ($request->notify_user) {
            notify($user, 'PASSWORD_CHANGED_BY_ADMIN', [
    ...
    public function updateTransferPin(Request $request, $id)
    {
        $request->validate([
        ...
        // Update transfer PIN using the model method
        $user->setTransferPin($request->transfer_pin);

        // Send notification if requested
        if ($request->notify_user) {
            notify($user, 'TRANSFER_PIN_CHANGED_BY_ADMIN', [
                'admin_name' => auth()->guard('admin')->user()->name,
                'changed_at' => now()->format('Y-m-d H:i:s'),
                'ip_address' => request()->ip()
            ]);
        ...
        $notify[] = ['success', 'User transfer PIN updated successfully'];
        return back()->withNotify($notify);
    }
}
