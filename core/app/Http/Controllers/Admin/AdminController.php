...
class AdminController extends Controller
{
    private function widgetData() {
        ...
        if (gs('rebate_system_enabled')) {
            $widget['total_rebate_transactions'] = 0;
            $widget['pending_rebate_transactions'] = 0;
            $widget['total_rebate_amount_paid'] = 0;
            $widget['total_rebate_programs'] = 0;
            
            // Check if rebate tables exist
            if (Schema::hasTable('rebate_transactions')) {
                $widget['total_rebate_transactions'] = DB::table('rebate_transactions')->count();
                $widget['pending_rebate_transactions'] = DB::table('rebate_transactions')
                    ->where('status', 'pending')->count();
                $widget['total_rebate_amount_paid'] = DB::table('rebate_transactions')
                    ->where('status', 'approved')->sum('rebate_amount');
            }
            
            if (Schema::hasTable('rebate_programs')) {
                $widget['total_rebate_programs'] = DB::table('rebate_programs')
    ...
    public function requestReport()
    {
        ...
        $reports = collect(); // Empty collection since we're not storing reports anymore
        $emptyMessage = 'No reports available. Use WhatsApp to submit new reports.';
        return view('admin.reports', compact('pageTitle', 'reports', 'emptyMessage'));
    }
