...
class AirtimeController extends Controller {
    ...
    public function fetchCountries() {
        ...
        // Ensure apiCountries is always an array to prevent template errors
    ...
    public function saveCountries(Request $request) {
        ...
        foreach ($requestCountries as $item) {
            // Skip if item is not an object or doesn't have required properties
            if (!is_object($item) || !isset($item->isoName)) {
                continue;
            }
    ...
    public function fetchOperatorsByISO($iso) {
        ...
        // Ensure we have a valid array of operators
        if (!is_array($reloadlySupportedOperators)) {
            $reloadlySupportedOperators = [];
        }
