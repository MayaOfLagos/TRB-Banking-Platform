...
class Email extends NotifyProcess implements Notifiable{
    ...
    */
	...
	protected function sendSmtpMail(){
		...
		$mailConfigRaw = gs('mail_config');
		$config = is_string($mailConfigRaw) ? json_decode($mailConfigRaw) : $mailConfigRaw;
        //Server settings
        $mail->isSMTP();
	...
	protected function sendLaravelMail(){
		\Illuminate\Support\Facades\Mail::html($this->finalMessage, function ($message) {
			$message->to($this->email, $this->receiverName)
					->subject($this->subject)
					->from($this->getEmailFrom()['email'], $this->getEmailFrom()['name']);
		});
	}
	...
	protected function sendSendGridMail(){
	    ...
	    $mailConfigRaw = gs('mail_config');
		$mailConfig = is_string($mailConfigRaw) ? json_decode($mailConfigRaw) : $mailConfigRaw;
	    $sendgrid = new SendGrid($mailConfig->appkey);
	...
	protected function sendMailjetMail()
	{
		$mailConfigRaw = gs('mail_config');
		$mailConfig = is_string($mailConfigRaw) ? json_decode($mailConfigRaw) : $mailConfigRaw;
	    $mj = new Client($mailConfig->public_key, $mailConfig->secret_key, true, ['version' => 'v3.1']);
	    $body = [
	        'Messages' => [
	            [
	                'From' => [
